...
class ExpressionParserSuit extends AnyFunSuite {
  ...
  test( "define program one" ) {
    ...
    program assign("lambda",{
      Lambda(Array("a"),(program lambdaBlock() expression(scope⇒{
        ...
        Return( Some(Add( Add( Add( scope.varRef( "a" ), 1 int ),
          MethodCall(
            foo1,"foo1",Array(
                Lambda(Array("j"),(scope.lambdaBlock() expression(scope⇒ {
                  Return(Some(Add(VarCall(scope.varRef("j"),"toInt",Array()),MethodCall(
                    foo1,"foo1",Array(Lambda(Array("z"),(scope.lambdaBlock() expression(scope⇒Return(Some(Add(VarCall(scope.varRef("z"),"toInt",Array()),122 int))))).block),"\"121L\"" str)
                  ) )))
                })).block)
              ,"\"12.1f\"" str
            )
          ...
          foo1,"foo1",Array(
            ...
            Lambda(Array("k"),(scope.lambdaBlock() expression(scope⇒{ Return(Some(VarCall(scope.varRef("k"),"toLong",Array())))})).block)
    ...
    program method(
      ...
      val case2 = scope.lambdaBlock().expression(s⇒Return(Some(Add(Add(s.varRef("a1"),s.varRef("a2")),s.varRef("a3")))))
      ...
      MatchCase(scope.varRef("i"),Array((12 int,case1.block),(MatchTuple(
      ...
      Lt(b.varRef("k"),100 int),b⇒Assign(b.varRef("k"),Add(b.varRef("k"),1 int))) expression(scope⇒{

      VarCall(scope.varRef("lambda"),"apply",Array(scope.varRef("j")))
    }) belongBlock() expression(scope⇒{
      ...
      val body = Lambda(Array("a","b","c"),(scope.lambdaBlock() updateVarDef("sum",IntType,Some(0 int)) expression(scope⇒{
        ...
        scope.varDef("lambda2",LambdaType(Some(TupleType(Array(IntType,IntType))),IntType),
          Some(Lambda(Array("a","b"),(scope.lambdaBlock() expression(scope⇒Return(Some(Add(scope.varRef("a"),scope.varRef("b")))))).block)))
      }) forLoopBlock(
        b⇒b.varDef("a",IntType,Some(0 int)),
        b⇒Lt(b.varRef("a"),100 int),
        b⇒Assign(b.varRef("a"),Add(b.varRef("a"),1 int))) expression(scope⇒

            Assign(scope.varRef("sum"),Add(scope.varRef("sum"),
              VarCall(scope.varRef("lambda2"),"apply",Array(scope.varRef("a"),scope.varRef("c")))))
      ...
      Return(Some(VarCall(scope.varRef("lambda1"),"apply",Array(1 int,2 int,3 int))))
    })
  ...
  test( "define xx empty program" ) {
    ...
    program method("xx") bodyBlock() expression(_⇒{
      ...
      Return(Some(Add(FuncCallChain(VarCall(scope.varRef("c","d","b","f"),"map",Array(Lambda(Array("k"),(scope lambdaBlock() expression (scope ⇒ {
        scope.varDef( "lambda", LambdaType( Some(TupleType( Array( IntType, LongType ) )), IntType ),
          Some( Lambda( Array( "a", "d" ), (scope.lambdaBlock( ) expression (scope ⇒ {
            Return( Some( Add( Add( scope.varRef( "k" ), scope.varRef( "d" ) ), scope.varRef( "a" ) ) ) )
          })).block ) ) )
      }) expression (scope ⇒ {
        Return( Some( FuncCallChain( VarCall( scope.varRef( "lambda" ), "apply", Array( scope.varRef( "k" ), 10 long ) ), List( MethodCall( None, "toString", Array( ) ) ) ) ) )
      })).block))),
        ...
        Lambda(Array("i"), (scope.lambdaBlock() expression(s⇒Return(Some(Add(s.varRef("i"),1 int))))).block),
      )),
    ...
    program.method("foo1") bodyBlock() updateVarDef("c",ClazzType("C"),None) expression(scope⇒{
      ...
      val caseCondition1 = MatchList(Array(Left("\"dd\"" str),Left(MatchClass(ClazzType("A"),Array(Right("dd"),Left(MatchList(Array(Left(1 int),Left(1 int),Right("x")))))))))
      val caseBlock1 = scope lambdaBlock() expression (_⇒Return(Some(1 int)))

      val caseCondition2 = MatchClass(ClazzType("Left"),Array(Left(12 int)))
      val caseBlock2 = scope lambdaBlock() expression (_⇒Return(Some(2 int)))

      val caseCondition3 = MatchClass(ClazzType("Array"),Array(Left(1 int),Left(2 int),Right("None")))
      ...
      val default = scope lambdaBlock() updateVarDef("d",IntType,Some(100 int)) expression(s⇒{
        ...
        Assign(s.varRef("d"),Add(s.varRef("d"),10 int))
      }) belongBlock() expression(s⇒{

        val case1 = s lambdaBlock() expression(_⇒Return(Some(10 int)))
        MatchCase(s.varRef("str"),Array((10 int,case1.block)),None)

      }) expression(s⇒Return(Some(s.varRef("d"))))


      scope.varDef("x",IntType,Some(MatchCase(scope.varRef("str"),Array(((caseCondition1,caseBlock1.block)),(caseCondition2,caseBlock2.block),(caseCondition3,caseBlock3.block)),Some(default.block))))
