...
import com.dongjiaqiang.jvm.dsl.api.`type`.{DslType, IntType, MapType, StringType}
...
class SymbolDefParserTestNestingSuite extends AnyFunSuite {
  ...
  test("define program no nesting") {
    ...
    //define method
    programScope statement 5 method ("xx",0,StringType,1)
    ...
    //define class
    programScope clazz ("Student",3) statement 3 field(
      "age",0,IntType,programScope
    ) field(
      "address",0,StringType,programScope
    ) method(
      "format",2,StringType,1
    ) statement 1 field(
      "sep",0,StringType,programScope
    )
    ...
    programScope method(
      ...
      "name",0,StringType,programScope
    ) body() field (
      "age",0,IntType,programScope
    ) field(
      "address",1,StringType,programScope
    )
  ...
  test("define program one level of nesting") {
    ...
    //define method
    programScope statement 5 method(
      "xx",0,StringType,1
    )
